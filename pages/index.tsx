...
const inset = "startContent" as undefined | 'gutter' | 'startDecorator' | 'startContent'
...
export default function Home() {
  const [currentMessage, setCurrentMessage] = React.useState<Schema['Message']>();
    ...
    <CssVarsProvider>
      <main>
        ...
        >
          ...
          <Button onClick={createMessage} sx={{ mt: 1 /* margin top */ }}>Post Message</Button>
          <Typography
            fontSize="sm"
            sx={{ alignSelf: 'center' }}
          >
        ...
        >
          ...
          <div key={inset || 'default'}>
            <ListItem>
              ...
              <ListItemContent>{currentMessage ? currentMessage.content : ''}
              </ListItemContent>
              <Typography level="body-sm">{new Date(currentMessage ? currentMessage.createdAt : '').toDateString()}</Typography>
            </ListItem>
          </div>
        </Sheet>
        <Sheet
            ...
            >
              {messages.map(message => <>
                <ListItem>
                  ...
                  <ListItemContent>{message.content}
                  </ListItemContent>
                  <Typography level="body-sm">{new Date(message.createdAt).toDateString()}</Typography>
                </ListItem>
